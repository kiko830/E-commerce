...
import {  type Basket, type Item } from "../../app/models/basket";
        ...
        addBasketItem: builder.mutation<Basket, { product: Product | Item ; quantity: number }>({
            ...
            onQueryStarted: async ({product, quantity},{dispatch, queryFulfilled}) => {
                ...
                try {
                    ...
                    if (isNewBasket) {
                        // If this is a new basket, we need to refetch the basket to get the basketId
            ...
            onQueryStarted: async ({productId, quantity}, {dispatch, queryFulfilled}) => {
                const patchResult = dispatch(
                    basketApi.util.updateQueryData('fetchBasket', undefined, (draft) => {
                        ...
                        if (itemIndex >= 0) {
                            const itemQuantity = draft.items[itemIndex].quantity - quantity;
                            if (itemQuantity > 0) {
